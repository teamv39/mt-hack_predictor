...
#### **Задача 7.1.5 (P1): PyTorch модуль для последовательностей телеметрии (требование стека) [ВЫПОЛНЕНО]**
...
* **Результаты эксперимента:**
  * **GRU-only holdout MAE:** 99.90с (обучение на 4434 строках, early stopping на эпохе 26/50).
  * **CatBoost-only holdout MAE:** 53.19с (24 hand-crafted фичи).
  * **Блендинг:** Не улучшает результат — лучший alpha=1.0 (чистый CatBoost). При alpha=0.85 MAE=57.31с (хуже CatBoost на 4.12с).
...
#### **Задача 7.1.8 (P0): Генерация документации кода PyDoc и портала (Критерий 4 формы сдачи) [ВЫПОЛНЕНО]**
* **Цель:** Предоставить полную сгенерированную документацию по исходному коду пакета `src/` в соответствии с пунктом 4 формы сдачи проекта.
* **Скрипт:** `ml/scripts/generate_pydoc.py`.
* **Результаты:**
  * Сгенерированы стандартные PyDoc HTML файлы для всех 28 модулей в директории `docs/pydoc/`.
  * Создан интерактивный стилизованный веб-портал `docs/pydoc/index.html` с навигацией по 5 подсистемам (`api`, `models`, `features`, `schemas`, `core`).
  * Написан обзорный документ `docs/pydoc.md` и добавлена команда `make ml-docs`.
* **Интерактивный просмотр:** `uv run python -m pydoc -p 8088` $\to$ `http://localhost:8088/src.html`.

---

#### **Задача 7.1.9 (P0): Замеры производительности ML-сервиса и паспорт производительности (Критерий 5) [ВЫПОЛНЕНО]**
* **Цель:** Подтвердить выполнение Критерия 5 ТЗ («Низкая задержка инференса < 1–2 с на поток ТС, пропускная способность без очередей, надежная деградация, предсказуемый холодный старт»).
* **Скрипт:** `ml/scripts/benchmark_service.py`.
* **Результаты верифицированных замеров:**
  * **Одиночный инференс:** CatBoost Fast — 1.31 мс (548 RPS); с расчетом TreeSHAP — 105.3 мс (9.2 RPS).
  * **Масштабируемость батчей:** $N=25$ (маршрут м3) — 1.74 мс (14 395 ТС/с); $N=50$ — 1.98 мс (25 271 ТС/с); $N=500$ (сектор) — 6.46 мс (77 403 ТС/с); $N=1000$ — 14.26 мс (70 133 ТС/с).
  * **Сетевой HTTP API (FastAPI):** `/health` p50 = 0.33 мс (2 917 RPS); `/predict` p50 = 1.90 мс (515 RPS); `/predict/batch` (N=50) p50 = 3.24 мс (15 297 ТС/с).
  * **Отказоустойчивость:** Failover на `HeuristicFallbackPredictor` за 11.1 мкс (90 265 оп/сек).
  * **Холодный старт:** 0.33 с, потребление памяти RAM (RSS) — 155.4 МБ.
  * **Покрытие всего флота Москвы (8 000 ТС):** Поток 1 600 ТС/с обрабатывается одним ядром CPU с загрузкой всего 4.2% (запас 23.7×).
* **Артефакты:** `docs/ml_performance.md`, `docs/benchmarks/ml_benchmark_results.json`, команда `make ml-benchmark`.
